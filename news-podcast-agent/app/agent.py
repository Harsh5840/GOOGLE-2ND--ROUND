...
NewsResearcher = LlmAgent(
    model=config.worker_model,
    name="NewsResearcher",
    description="Finds and summarizes the latest local news for a given city.",
    instruction="""
    You are a local news researcher. Use the local_news_tool to fetch the latest news for the user's specified city. Summarize the most important stories in a concise, engaging way for a podcast audience.
    """,
    tools=[local_news_tool],
    output_key="local_news_summary",
)

PodcastScripter = LlmAgent(
    model=config.worker_model,
    name="PodcastScripter",
    description="Creates a podcast script from summarized news, matching the user's desired length.",
    instruction="""
...
PodcastProducer = LlmAgent(
    model=config.worker_model,
    name="PodcastProducer",
    description="Converts the podcast script into an audio file using TTS.",
    instruction="""
    You are a podcast producer. Use the text_to_speech_tool to convert the podcast script into an audio file. 
    Use an expressive, natural-sounding voice (such as a Studio or Wavenet voice) and expressive prosody to make the podcast engaging and pleasant to listen to. Return the file path or a link to the generated audio.
    """,
    tools=[text_to_speech_tool],
    output_key="podcast_audio_file",
)

PodcastPipeline = SequentialAgent(
    name="PodcastPipeline",
    description="Pipeline: research news, script podcast, generate audio.",
    sub_agents=[
        NewsResearcher,
        PodcastScripter,
        PodcastProducer,
...
PodcastOrchestrator = LlmAgent(
    model=config.worker_model,
    name="PodcastOrchestrator",
    description="Main agent for podcast creation. Prompts user for city and podcast length, then runs the podcast pipeline.",
    instruction="""
    You are a podcast creation assistant. Ask the user for:
    1. The city for which they want local news.
    2. The desired length of the podcast (in minutes).
    Once you have both, pass them to the PodcastPipeline agent to generate the podcast.
    """,
    sub_agents=[PodcastPipeline],
)

root_agent = PodcastOrchestrator
