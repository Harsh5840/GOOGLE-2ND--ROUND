...
def aggregate_mood(unified_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Robust to bad input and always returns a safe fallback.
    """
    try:
        ...
        # Only use valid floats for mood score
        source_scores = [safe_score(twitter_score), safe_score(reddit_score), safe_score(news_score), safe_score(google_score), safe_score(maps_score)]
        valid_scores = [s for s in source_scores if isinstance(s, (int, float))]
        mood_score = sum(valid_scores) / len(valid_scores) if valid_scores else 0.0
        if mood_score > 0.3:
            mood_label = "happy"
        elif mood_score < -0.3:
            mood_label = "tense"
        elif safe_score(maps_score) < 0:
            mood_label = "busy"
        else:
            mood_label = "neutral"
        return {
            ...
            "source_breakdown": {
                "twitter": {"score": round(safe_score(twitter_score), 2), "top_keywords": twitter_keywords},
                "reddit": {"score": round(safe_score(reddit_score), 2), "top_keywords": reddit_keywords},
                "news": {"score": round(safe_score(news_score), 2), "top_keywords": news_keywords},
                "google_search": {"score": round(safe_score(google_score), 2), "top_keywords": google_keywords},
                "maps": {"score": round(safe_score(maps_score), 2), "top_keywords": maps_keywords},
            }
        }
    except Exception as e:
        import traceback
        from shared.utils.logger import log_event
        log_event("Mood", f"Error in aggregate_mood: {e}\n{traceback.format_exc()}")
        return {
            "mood_label": "neutral",
            "mood_score": 0.0,
            "events": [],
