...
def create_analysis_agent():
    ...
    return Agent(
        model="gemini-2.0-flash-001",
        name="analysis_agent",
        instruction="You are the Analysis Agent. Use the available tools to gather information, but once you have enough, answer the user's question in plain text. Do not keep calling tools if you can answer.",
        tools=[]
    )
...
async def run_analysis_agent(query: str, research_results: dict = None, data_results: dict = None, context: dict = None) -> dict:
    ...
    print(f"[DEBUG] run_analysis_agent returning: {response_text}")
