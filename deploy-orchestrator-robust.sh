...
# Deploy Orchestrator with robust main.py (handles import errors gracefully)
set -e

echo "🚀 Deploying City Project Orchestrator (Robust Version)..."
...
# Copy orchestrator files but use robust main.py
cp -r apps/orchestrator/* "$TEMP_DIR/"
cp apps/orchestrator/main_robust.py "$TEMP_DIR/main.py"
...
# Run the application
CMD ["uvicorn", "main_robust:app", "--host", "0.0.0.0", "--port", "8080"]
EOF
