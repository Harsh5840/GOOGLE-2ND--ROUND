...
# Deploy Orchestrator with minimal main.py (guaranteed to work)
set -e

echo "🚀 Deploying City Project Orchestrator (Minimal Version)..."

# Create a temporary deployment directory
TEMP_DIR=$(mktemp -d)
echo "📁 Created temp directory: $TEMP_DIR"

# Copy only the minimal files
echo "📋 Copying minimal files..."
cp apps/orchestrator/main_minimal.py "$TEMP_DIR/main.py"
cp apps/orchestrator/requirements_minimal.txt "$TEMP_DIR/requirements.txt"
